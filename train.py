...
parser = ArgumentParser(description='HIDIDUN')
...
my_loader = DataLoader(dataset=MyDataset(), batch_size=batch_size, num_workers=0, pin_memory=True)

optimizer = torch.optim.Adam([{'params': model.parameters(), 'initial_lr': learning_rate}], lr=learning_rate)
scheduler = torch.optim.lr_scheduler.MultiStepLR(optimizer, milestones=[400, 500, 550], gamma=0.1,
                                                 last_epoch=start_epoch - 1)

model_dir = './%s/mark5_%d_ratio_%.2f_layer_%d_block_%d' % (args.model_dir, channel_number, cs_ratio, N_p, B)
log_path = './%s/mark5_%d_ratio_%.2f_layer_%d_block_%d.txt' % (args.log_dir, channel_number, cs_ratio, N_p, B)
